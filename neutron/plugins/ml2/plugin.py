...
class Ml2Plugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def _validate_provider_create(self, context, network):
        if not attributes.is_attr_set(network.get(mpnet.SEGMENTS)):
            LOG.error(_('---> KYLE: MULTI PROVIDER NETWORK NOT FOUND!!!!'))
            return

        LOG.error(_('---> KYLE: MULTI PROVIDER NETWORK FOUND!!!!'))
        segments = []
        for segment in network[mpnet.SEGMENTS]:
            ...
            elif (attributes.is_attr_set(segment.get(provider.PHYSICAL_NETWORK))
                or attributes.is_attr_set(segment.
    ...
    def _extend_network_dict_provider(self, context, network):
        ...
        if not segments:
            ...
            LOG.error(_('----> Single network'))
            segment = segments[0]
            LOG.error(_('----> NETWORK_TYPE: %s' % segment[api.NETWORK_TYPE]))
    ...
    def _convert_to_multi_provider(self, network):
        ...
        if any(attributes.is_attr_set(network.get(f))
            ...
            LOG.error(_('----> KYLE Converting to MULTI PROVIDER'))
            network[mpnet.SEGMENTS] = [
                {provider.NETWORK_TYPE: network[provider.NETWORK_TYPE],
                 provider.PHYSICAL_NETWORK: network[provider.
                                                    PHYSICAL_NETWORK],
                 provider.SEGMENTATION_ID: network[provider.SEGMENTATION_ID]}]
            LOG.error(_('----> dumping network: %s' % network))
            del network[provider.NETWORK_TYPE]
            del network[provider.PHYSICAL_NETWORK]
            del network[provider.SEGMENTATION_ID]
            return False
        if attributes.is_attr_set(mpnet.SEGMENTS):
            return True

    def create_network(self, context, network):
        net_data = network['network']
        self._convert_to_multi_provider(net_data)
        segments = self._validate_provider_create(context, net_data)
        ...
        with session.begin(subtransactions=True):
            ...
            # to TypeManager.
            if (net_data.get(mpnet.SEGMENTS)):
                for segment in segments:
                    LOG.error(_('SEGMENT: %s' % segment))
        ...
        LOG.error(_('----> Returning created network of: %s' % result))
        return result
